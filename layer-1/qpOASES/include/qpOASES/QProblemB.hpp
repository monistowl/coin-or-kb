 ...
 *
 * @algorithm Solves KKT conditions directly when active set is known.
 *            Uses Cholesky factorization of reduced Hessian.
 *
 * @see QProblem for QPs with general linear constraints
 *
...
/**
 * @brief Box-constrained QP solver: min 0.5*x'*H*x + g'*x s.t. lb <= x <= ub
 *
 * Base class for qpOASES solvers. Handles QPs with only simple bounds.
 * Provides shared functionality for factorization, active set management,
 * and KKT system solving.
 *
 * Workflow:
 * 1. Construct: QProblemB(nV) for nV variables
 * 2. First solve: init(H, g, lb, ub, nWSR)
 * 3. Hot start: hotstart(g_new, lb_new, ub_new, nWSR)
 *
 * @invariant nWSR controls maximum iterations (real-time guarantee)
 *
 * @author Hans Joachim Ferreau, Andreas Potschka, Christian Kirches
 * @version 3.2
 * @date 2007-2017
