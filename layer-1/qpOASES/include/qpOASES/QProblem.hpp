 ...
 * @math s.t. lbA <= A * x <= ubA  (constraints)
 * @math      lb  <= x <= ub       (bounds)
 *
 * Key methods:
 ...
 *
 * @algorithm Online Active Set Strategy:
 ...
 *
 * @see QProblemB for box-constrained QPs (no A matrix)
 * @see SQProblem for varying Hessian
 *
...
/**
 * @brief QP solver with bounds and general linear constraints
 *
 * Extends QProblemB to handle QPs with constraint matrix A:
 * @math min  0.5 * x' * H * x + g' * x
 * @math s.t. lbA <= A * x <= ubA
 * @math      lb  <= x <= ub
 *
 * Main workflow:
 * 1. Construct: QProblem(nV, nC) for nV variables, nC constraints
 * 2. First solve: init(H, g, A, lb, ub, lbA, ubA, nWSR)
 * 3. Subsequent: hotstart(g_new, lb_new, ub_new, lbA_new, ubA_new, nWSR)
 *
 * @invariant H must be positive semi-definite for convex QP
 * @invariant nWSR limits iterations for real-time guarantees
 *
 * @author Hans Joachim Ferreau, Andreas Potschka, Christian Kirches
 * @version 3.2
 * @date 2007-2017
