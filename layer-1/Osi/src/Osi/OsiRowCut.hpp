...
/**
 * @brief Row cut representing linear inequality lb <= a'x <= ub
 *
 * A row cut specifies:
 * - Lower bound (lb): -infinity for >= constraints
 * - Upper bound (ub): +infinity for <= constraints
 * - Row elements: sparse vector of (column_index, coefficient) pairs
 *
 ...
 * - 'R' (range): lb <= a'x <= ub
 *
 * @see CoinPackedVector for sparse row representation
 */
