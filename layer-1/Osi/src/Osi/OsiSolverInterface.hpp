...
/**
 * @brief Abstract base class for LP/MIP solver interface
 *
 * This pure virtual class defines the standard interface that all
 * OSI-compatible solvers must implement. It provides:
 *
 * - **Solve methods**: initialSolve(), resolve(), branchAndBound()
 * - **Status queries**: isProvenOptimal(), isProvenInfeasible()
 * - **Model access**: getNumRows(), getNumCols(), getMatrixByRow()
 * - **Solution access**: getColSolution(), getObjValue(), getRowPrice()
 * - **Problem modification**: addRow(), addCol(), setColBounds()
 * - **Warm start**: getWarmStart(), setWarmStart()
 * - **Cut management**: applyCuts()
 *
 * @invariant Query pointers become invalid after model changes or solve calls
 * @invariant Empty problems return 0 rows/cols and NULL pointers
 *
 * @note Concrete implementations: OsiClpSolverInterface, OsiCpxSolverInterface,
 *       OsiGrbSolverInterface, OsiGlpkSolverInterface, etc.
 */
...
public:
  ...
  /**
   * @name Solve methods
   * @brief Core methods for solving LP and MIP problems
   */
  //@{

  /**
   * @brief Solve LP relaxation from scratch
   *
   * Use this for the first solve of a problem. Sets up internal data
   * structures and performs a cold start. After this, use resolve()
   * for subsequent solves.
   */
  virtual void initialSolve() = 0;

  /**
   * @brief Re-solve LP after problem modifications
   *
   * Uses warm start information from previous solve when possible.
   * More efficient than initialSolve() for incremental changes.
   * Call initialSolve() first; use resolve() for all subsequent solves.
   */
  virtual void resolve() = 0;

  /**
   * @brief Invoke solver's built-in MIP algorithm
   *
   * Performs branch-and-bound (or branch-and-cut) to solve the
  ...
  */
  ...
  //---------------------------------------------------------------------------
  /**
   * @name Solution status queries
   * @brief Check how the solve terminated and solution validity
   *
   * After calling initialSolve(), resolve(), or branchAndBound(),
   * use these methods to determine the outcome.
   */
  //@{

  /** @brief Solver abandoned due to numerical difficulties */
  virtual bool isAbandoned() const = 0;

  /** @brief Optimal solution found and proven */
  virtual bool isProvenOptimal() const = 0;

  /** @brief Problem proven to have no feasible solution */
  virtual bool isProvenPrimalInfeasible() const = 0;

  /** @brief Problem proven unbounded (dual infeasible) */
  virtual bool isProvenDualInfeasible() const = 0;

  /** @brief Primal objective cutoff reached */
  virtual bool isPrimalObjectiveLimitReached() const;

  /** @brief Dual objective cutoff reached */
  virtual bool isDualObjectiveLimitReached() const;

  /** @brief Iteration limit hit before convergence */
  ...
  */
   ...
   * @brief Access primal solution, dual solution, and solve statistics
   *
   * These return pointers to solver-internal data. Pointers become
   * invalid after any model modification or solve call.
   */
  //@{

  /**
   * @brief Get primal variable values (x)
   * @return Array of size getNumCols() with solution values
   */
  virtual const double *getColSolution() const = 0;

  /**
   * @brief Get primal values clamped to variable bounds
   * @return Array of size getNumCols() with bounded solution values
   */
  virtual const double *getStrictColSolution();

  /**
   * @brief Get dual variable values (row prices / shadow prices)
   * @return Array of size getNumRows() with dual values (pi)
   */
  virtual const double *getRowPrice() const = 0;

  /**
   * @brief Get reduced costs (c - A'*pi)
   * @return Array of size getNumCols() with reduced costs
   */
  virtual const double *getReducedCost() const = 0;

  /**
   * @brief Get row activity levels (A*x)
   * @return Array of size getNumRows() with constraint LHS values
   */
  virtual const double *getRowActivity() const = 0;

  /**
   * @brief Get optimal objective function value
   * @return Objective value c'*x at current solution
   */
  virtual double getObjValue() const = 0;

  /**
   * @brief Get iteration count from last solve
   * @return Number of simplex iterations or equivalent
   */
