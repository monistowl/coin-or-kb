 ...
 * 3. Solver applies cuts via OsiSolverInterface::applyCuts()
 *
 * @see OsiRowCut for linear inequality cuts
 * @see OsiColCut for variable bound cuts
 * @see OsiSolverInterface::applyCuts for adding cuts to solver
 */
...
/**
 * @brief Container class for row cuts and column cuts
 *
 * Manages a collection of cutting planes for use in branch-and-cut.
 * Provides STL-style iterators for traversing all cuts.
 *
 * @see OsiSolverInterface::applyCuts for applying cuts to a model
 */
...
public:
  ...
  bool insertIfNotDuplicate(OsiRowCut &rc, CoinAbsFltEq treatAsSame = CoinAbsFltEq(1.0e-12));
  /** \brief Insert a row cut unless it is a duplicate - cut may get sorted.
