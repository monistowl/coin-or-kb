 ...
 * - scale(): Apply row/column scaling
 *
 * @see ClpPackedMatrix for standard sparse matrix (wraps CoinPackedMatrix)
 * @see ClpNetworkMatrix for pure network structure
 * @see ClpPlusMinusOneMatrix for ±1 coefficient matrices
 * @see ClpGubMatrix for generalized upper bound structure
 * @see CoinPackedMatrix in CoinUtils for sparse storage format
 */
