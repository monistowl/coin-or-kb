 ...
 *
 * @math Reliability branching: Use pseudo-costs ψ_j unless branched on
 * variable < η_rel times, then strong branch to improve estimate.
 * Balances accuracy vs cost of strong branching.
 *
 * @complexity Strong branching: O(k × LP_iterations) per node.
 * Pseudo-costs: O(n) after warmup. Reliability: adaptive O(k') where k' ≤ k.
 *
 * @ref Achterberg, Koch & Martin, "Branching rules revisited",
 ...
 *
 * @see CbcBranchDefaultDecision for simple selection
 * @see CbcBranchDynamicDecision for pseudocost-based selection
 * @see CbcBranchingObject for what gets compared
 * @see OsiChooseVariable for variable selection integration
 *
