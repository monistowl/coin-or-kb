 ...
 * derivatives.
 *
 * @algorithm Tape-Based Operation Recording:
 ...
 * Tapes are identified by short integer IDs (0-32767).
 *
 * @math Tape interpretation:
 ...
 * Reverse mode propagates adjoints: v̄_j += (∂φ_i/∂v_j)·v̄_i (in reverse order)
 *
 * @complexity Recording: O(ops) time and space
 * Forward evaluation: O(ops) per direction
 * Reverse evaluation: O(ops) per adjoint
 *
 * @see adolc.h for main include
 * @see interfaces.h for forward/reverse mode API
 * @see adtb_types.h for adouble/pdouble types
 */
