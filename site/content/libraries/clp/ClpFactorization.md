...
FTRAN solves: d = B^{-1}$a_{j}$ (pivot column for primal simplex)
BTRAN solves: $π^{T}$ = $$c_{B}$^{T}$ B^{-1} (dual prices for reduced cost calculation)
Update: $B_{new}$ = $B_{old}$ + ($a_{j}$ - $Be_{i}$)$$e_{i}$^{T}$ for basis change at position i.
