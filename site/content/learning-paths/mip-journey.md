...
You should have completed [LP Fundamentals](/coin-or-kb/learning-paths/lp-fundamentals/) or be comfortable with:
...
In COIN-OR, MIP is solved by [Cbc](/coin-or-kb/libraries/cbc/) (COIN-OR Branch and Cut). It uses [Clp](/coin-or-kb/libraries/clp/) for LP relaxations and [Cgl](/coin-or-kb/libraries/cgl/) for cut generation.
...
<img src="/coin-or-kb/diagrams/branch-and-bound-tree.svg" alt="Branch-and-bound tree example" class="algorithm-diagram">
</div>
...
`CbcModel` manages the tree. `CbcNode` represents nodes. Tree traversal strategies are in `CbcTree` and `CbcCompare*` classes. See [branch-and-bound tree search](/coin-or-kb/algorithms/branch-and-bound-tree-search/).
...
The [Cgl library](/coin-or-kb/libraries/cgl/) provides cut generators. `CbcModel` calls them via `CglCutGenerator`. See the [lift-and-project algorithm](/coin-or-kb/algorithms/lift-and-project-cutting-planes/).
...
Cbc's branching strategies are in `CbcBranch*` classes. `CbcBranchDynamic` implements reliability branching. See [branching variable selection](/coin-or-kb/algorithms/branching-variable-selection-strategies/).
...
Cbc heuristics inherit from `CbcHeuristic`. See `CbcHeuristicFPump` for feasibility pump and `CbcHeuristicRINS` for RINS. The [primal heuristics framework](/coin-or-kb/algorithms/mip-primal-heuristics-framework/) documents the design.
...
The main loop is in `CbcModel::branchAndBound()`. See [branch-and-cut algorithm](/coin-or-kb/algorithms/branch-and-cut-bc-for-mixed-integer-programming/).
...
- **[Browse Cbc source](/coin-or-kb/browser/?library=Cbc)** - See the implementation
- **[Cutting plane algorithms](/coin-or-kb/algorithms/?category=cutting-planes)** - Deep dive on specific cuts
