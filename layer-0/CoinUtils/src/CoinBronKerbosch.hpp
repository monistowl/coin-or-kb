 ...
 * @date 03/27/2020
 *
 * @algorithm Bron-Kerbosch with pivoting for weighted cliques:
 ...
 *
 * @math Clique: complete subgraph where all vertices are pairwise adjacent.
 *   Weight threshold: only enumerate cliques with Σw(v) > minWeight.
 *   Pivoting strategies: degree, weight, or modified degree×weight.
 *
 * @complexity Time: O(3^(n/3)) worst case for all maximal cliques
 *   With pivoting: much faster in practice, especially for sparse graphs.
 *   Space: O(n) for recursion stack
 *
 * @ref Bron, Kerbosch (1973). "Algorithm 457: Finding all cliques of an
 *      undirected graph". CACM 16(9):575-577.
 * @ref Tomita, Tanaka, Takahashi (2006). "The worst-case time complexity
