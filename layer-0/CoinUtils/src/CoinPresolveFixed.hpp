...
/**
 * @file CoinPresolveFixed.hpp
 * @brief Remove fixed variables and make variables fixed
 *
 * @algorithm Fixed Variable Removal:
 ...
 *
 * @math Substitution: Ax = b with x_j = v becomes A'x' = b - a_j·v
 *   where A' is A without column j, x' is x without x_j
 *   Postsolve: restore x_j = v in solution
 *
 * @complexity Time: O(nnz(column_j)) per fixed variable
