 ...
 *
 * @see CoinPackedVectorBase for the read-only interface
 * @see CoinShallowPackedVector for non-owning sparse vector reference
 * @see CoinIndexedVector for sparse vector with dense backing array
 */
...
/**
 * @brief Returns the dot product of two sorted CoinPackedVector objects
 *
 * The vectors should be sorted in ascending order of indices.
 *
 * @algorithm Merge-style sparse dot product
 * @math Computes x·y = Σ x_i * y_i where x and y share index i
 *       Uses sorted merge to find common indices in linear time.
 * @complexity O(k1 + k2) where k1, k2 are the number of nonzeros
