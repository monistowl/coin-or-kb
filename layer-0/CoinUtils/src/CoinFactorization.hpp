 ...
 * @math Decomposes basis matrix B = L·U where:
 ...
 *
 * @complexity Factorization: O(nnz * fill) where fill depends on pivot order
 ...
 * @ref Suhl, Suhl (1990). "Computing Sparse LU Factorizations for
 *      Large-Scale Linear Programming Bases". ORSA J. Computing 2:325-335.
 *
 * @author John Forrest
 * @see CoinDenseFactorization for dense matrices
 * @see CoinOslFactorization for OSL-derived implementation
 */
...
public:
  ...
  */
  ...
  //@{
  /**
   * @brief Factorize basis matrix from LP basic variable indicators
   *
   * Performs LU factorization B = L·U of the basis matrix defined by the
   * basic columns of the constraint matrix.
   *
   * @param matrix Full constraint matrix A
   * @param rowIsBasic Array marking basic rows (>=0 means basic)
   * @param columnIsBasic Array marking basic columns (>=0 means basic)
   * @param areaFactor Multiplier for memory allocation (0 = auto)
   * @return 0 = success, -1 = singular, -2 = too many basic, -99 = memory
   *
   * @algorithm Sparse LU with Markowitz pivot selection
   * @math Chooses pivot (i,j) minimizing (r_i - 1)(c_j - 1) where r_i, c_j
   *       are row and column counts in active submatrix. This heuristic
   *       minimizes expected fill-in during Gaussian elimination.
   * @complexity O(m^2) worst case, typically O(m * nnz_avg) for sparse matrices
   *
   * @post On success: rowIsBasic/columnIsBasic contain pivot row assignments
   * @post On singular: basic variables thrown out have value -1
   */
  ...
  */
  ...
  /**
   * @brief Update factorization after basis change (rank-one update)
   *
   * Updates the LU factorization when one column of the basis is replaced.
   * This is the key operation during simplex pivoting - much faster than
   * refactorizing from scratch.
   *
   * @param regionSparse The column entering the basis (after FTRAN)
   * @param pivotRow Row index of the leaving variable
   * @param pivotCheck Expected pivot value for accuracy verification
   * @param checkBeforeModifying If true, validate before modifying factors
   * @param acceptablePivot Minimum acceptable |pivot| for stability
   * @return 0=OK, 1=Probably OK, 2=singular, 3=no room
   *
   * @algorithm Forrest-Tomlin update (when doForrestTomlin_=true) or PFI
   * @math For basis change B_new = B_old + (a_q - a_p) · e_p':
   *       Forrest-Tomlin maintains L and updates U with spike elimination.
   *       PFI (Product Form of Inverse) appends eta matrices.
   * @complexity O(nnz(column)) for FT update; PFI grows over iterations
   *
   * @pre regionSparse contains FTRAN result of entering column
   * @post Factorization updated to represent new basis
   *
   * @ref Forrest, Tomlin (1972). Mathematical Programming 2:263-278.
   * @see forrestTomlin() to check which update method is active
   */
  ...
  */
  ...
  //@{
  /**
   * @brief Forward transformation (FTRAN): solve Bx = b
   *
   * Solves the system Bx = b where B is the factorized basis matrix.
   * Uses the LU factors: first solves Ly = b, then Ux = y.
   * Attempts Forrest-Tomlin update if space available.
   *
   * @param regionSparse Work vector (must start as zero, returns zero)
   * @param regionSparse2 Right-hand side b on input, solution x on output
   * @return Number of nonzeros in result, negative if no room for FT update
   *
   * @algorithm FTRAN with optional Forrest-Tomlin update preparation
   * @math Solves B·x = b where B = L·U:
   *       Step 1: Solve L·y = b (forward substitution)
   *       Step 2: Solve U·x = y (backward substitution)
   * @complexity O(nnz(L) + nnz(U)) - exploits sparsity
   *
   * @note Preserves packed/unpacked format of regionSparse2
   * @see updateColumnTranspose for BTRAN (B'y = c)
   */
  int updateColumnFT(CoinIndexedVector *regionSparse,
  ...
  /**
   * @brief Backward transformation (BTRAN): solve B'y = c
   *
   * Solves the transposed system B'y = c where B is the factorized basis.
   * Uses transposed LU factors: first solves U'z = c, then L'y = z.
   *
   * @param regionSparse Work vector (must start as zero, returns zero)
   * @param regionSparse2 Right-hand side c on input, solution y on output
   * @return Number of nonzeros in result
   *
   * @algorithm BTRAN using transposed triangular solves
   * @math Solves B'·y = c where B = L·U, so B' = U'·L':
   *       Step 1: Solve U'·z = c (forward substitution on U')
   *       Step 2: Solve L'·y = z (backward substitution on L')
   * @complexity O(nnz(L) + nnz(U)) - exploits sparsity
   *
   * @note Essential for computing reduced costs (pi = c_B · B^{-1})
   * @note Preserves packed/unpacked format of regionSparse2
   * @see updateColumnFT for FTRAN (Bx = b)
   */
