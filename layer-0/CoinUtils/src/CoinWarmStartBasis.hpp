 ...
 *
 * @algorithm Simplex Basis Warm Start:
 ...
 *
 * @math Basis matrix B is m×m submatrix of A with basic columns.
 *   Basic solution: x_B = B⁻¹b, x_N = 0 (or at bounds)
 *   Warm start restores this basis without recomputing from scratch.
 *
 * @complexity Space: O((m+n)/4) bytes using 2-bit packing (4 vars per byte)
 ...
 *
 * @see CoinWarmStart for the abstract base class
 * @see CoinWarmStartDual for dual-only warm start
 */
