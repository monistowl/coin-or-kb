...
/**
 * @file CoinPresolveDupcol.hpp
 * @brief Detect and remove duplicate columns and rows
 *
 * @algorithm Duplicate Detection via Random Hashing:
 ...
 *
 * @math Column equivalence: a_*j = a_*k (element-wise)
 *   Combined variable: x_new = x_j + x_k
 *   Postsolve: split x_new* back to feasible (x_j*, x_k*)
 *
 * @complexity Time: O(nnz) for hashing, O(candidates × nnz) for verification
