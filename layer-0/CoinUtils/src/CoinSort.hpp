 ...
 *
 * @algorithm Parallel Array Sorting:
 ...
 *
 * @complexity CoinSort_2/3: O(n log n) average, O(n²) worst
 *   CoinShortSort_2: O(n log n) but optimized for small n
 *   Space: O(n) for tuple approach, O(log n) stack for EKK
 *
 * @see CoinPackedVector for typical use case (sorting indices with values)
 */
