 ...
 *
 * @algorithm Floating-Point Comparison:
 ...
 *
 * @see CoinFinite.hpp for infinity/NaN handling utilities
 */
...
/**
 * @brief Equality to a scaled (relative) tolerance
 *
 * Operands are considered equal if their difference is within a scaled
 * epsilon calculated as epsilon_*(1+max(|f1|,|f2|)).
 *
 * This is preferred over absolute tolerance when comparing values that
