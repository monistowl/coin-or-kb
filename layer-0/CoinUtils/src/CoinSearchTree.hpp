 ...
 *
 * @algorithm Branch-and-Bound Tree Search:
 ...
 *
 * @complexity Node operations: O(log n) insertion, O(log n) extraction
 *   where n = number of open nodes. Uses heap-based priority queue.
 *   Sibling nodes stored together for efficient processing.
 *
 * @see CoinTreeNode for node data interface
 * @see CoinTreeSiblings for sibling node grouping
 */
