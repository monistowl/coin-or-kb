 ...
 *
 * @algorithm Duff's device - Tom Duff's loop unrolling technique that uses switch
 *            statement fall-through to handle remainder elements, reducing loop
 *            overhead from O(n) branches to O(n/8) branches.
 *
 * @complexity Summary of major functions:
 ...
 *
 * @ref Duff, T. (1983). "Duff's Device" - Usenet comp.lang.c posting.
 *      Originally for copying to memory-mapped display registers.
 *
 * @see CoinSort.hpp for sorting functions
 * @see CoinError.hpp for exception handling
 */
...
/**
 * @brief Copy array elements using Duff's device for ~2x speedup
 *
 * Copies elements from source to destination array, handling overlapping
 * arrays correctly (copies backwards when destination > source).
 *
 * @tparam T Element type (must be copy-assignable)
 * @param from Pointer to source array
 * @param size Number of elements to copy
 * @param to Pointer to destination array
 *
 * @algorithm Duff's device with 8-element unrolling, direction-aware for overlap safety
 * @complexity O(n) time, O(1) space - executes n/8 loop iterations
 *
 * @note Throws CoinError if size < 0 (in debug builds)
 * @see CoinDisjointCopyN for faster non-overlapping copy
 */
template < class T >
inline void
...
/**
 * @brief Copy array elements given iterator-style range
 *
 * Copies elements from [first, last) to destination, handling overlapping
 * arrays correctly.
 *
 * @tparam T Element type (must be copy-assignable)
 * @param first Pointer to first source element
 * @param last Pointer past last source element
 * @param to Pointer to destination array
 *
 * @note Size computed as (last - first), cast to CoinBigIndex
 * @see CoinCopyN for count-based interface
 */
template < class T >
inline void
...
/**
 * @brief Copy non-overlapping arrays with 8-element loop unrolling
 *
 * Optimized copy for arrays that do not overlap. Faster than CoinCopyN
 * when overlap checking is not needed.
 *
 * @tparam T Element type (must be copy-assignable)
 * @param from Pointer to source array
 * @param size Number of elements to copy
 ...
 * @note For POD types, CoinMemcpyN may be faster
 * @see CoinCopyN for overlap-safe copy
 */
template < class T >
inline void
...
/**
 * @brief Allocate and copy array, returning null if source is null
 *
 * @tparam T Element type
 * @param array Source array (may be null)
 * @param size Number of elements to copy
 * @return Newly allocated copy, or NULL if array was NULL
 *
 * @note Caller owns returned memory (delete[])
 */
template < class T >
inline T *
...
/**
 * @brief Allocate array and copy partial content from source
 *
 * @tparam T Element type
 * @param array Source array (may be null)
 * @param size Size of array to allocate
 * @param copySize Number of elements to copy from source (must be <= size)
 * @return Newly allocated array, or NULL if array was NULL and size is 0
 *
 * @note Caller owns returned memory (delete[])
 */
template < class T >
...
/**
 * @brief Duplicate a C string, returning NULL if input is NULL
 *
 * @param name Source string (may be null)
 * @return Newly allocated copy, or NULL if name was NULL
 *
 * @note Caller owns returned memory (free())
 */
 ...
 *
 * Portable max function introduced to avoid inconsistent compiler handling
 * of std::max across platforms.
 *
 * @tparam T Type supporting operator<
 ...
 *
 * Portable min function introduced to avoid inconsistent compiler handling
 * of std::min across platforms.
 *
 * @tparam T Type supporting operator<
 * @param x1 First value
 * @param x2 Second value
 ...
 *
 * Portable abs function introduced to avoid inconsistent compiler handling
 * of std::abs across platforms.
 *
 * @tparam T Numeric type supporting negation and comparison with 0
 ...
 *
 * Each instance maintains its own seed state, making it safe for
 * multi-threaded use when each thread has its own instance.
 * Uses a linear congruential generator for platform-independent
 * reproducible sequences.
 *
 * @see CoinDrand48 for global (non-thread-safe) random numbers
 */
#if defined COIN_OWN_RANDOM_32
