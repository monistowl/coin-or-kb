 ...
 *
 * @math Approximate external degree:
 ...
 * @ref George, Liu (1989). "The Evolution of the Minimum Degree Ordering
 ...
 *
 * @see colamd.h for column ordering of unsymmetric matrices
 * @see cholmod.h for sparse Cholesky factorization using AMD
 */
...
/**
 * @brief Compute fill-reducing ordering of a sparse symmetric matrix
 *
 * Computes a permutation P that reduces fill-in during Cholesky factorization.
 * If A is unsymmetric, AMD operates on A+A'. The matrix is given in compressed
 * sparse column (CSC) format.
 *
 * @param n Matrix dimension (A is n-by-n). Must be >= 0.
 * @param Ap Column pointers array of size n+1. Ap[j] gives the start of column j.
 * @param Ai Row indices array of size Ap[n]. Contains row indices for each column.
 * @param P Output permutation array of size n. P[k]=i means row i is the k-th pivot.
 * @param Control Optional control parameters of size AMD_CONTROL. NULL uses defaults.
 * @param Info Optional output statistics of size AMD_INFO. May be NULL.
 * @return AMD_OK on success, AMD_OK_BUT_JUMBLED if input had unsorted/duplicate
 *         entries, AMD_INVALID for bad input, AMD_OUT_OF_MEMORY on allocation failure.
 *
 * @see amd_l_order() for 64-bit integer version
 * @see amd_defaults() to initialize Control array
 */
int amd_order
(
    int32_t n,
...
/**
 * @brief Compute fill-reducing ordering (64-bit integer version)
 *
 * Same as amd_order() but uses 64-bit integers for large matrices.
 *
 * @see amd_order() for full parameter documentation
 */
int amd_l_order
(
    int64_t n,
...
/**
 * @brief Validate a sparse matrix for use with AMD ordering
 *
 * Checks whether a sparse matrix in CSC format is valid input for amd_order.
 * For AMD, the matrix must be square (n_row == n_col).
 *
 * @param n_row Number of rows in the matrix
 * @param n_col Number of columns in the matrix
 * @param Ap Column pointers array of size n_col+1
 * @param Ai Row indices array of size Ap[n_col]
 * @return AMD_OK if valid, AMD_OK_BUT_JUMBLED if valid but has unsorted/duplicate
 *         entries, AMD_INVALID if the matrix cannot be used with amd_order
 *
 * @see amd_order()
 */
int amd_valid
...
/**
 * @brief Validate a sparse matrix (64-bit integer version)
 * @see amd_valid() for full documentation
 */
int amd_l_valid
(
    int64_t n_row,
...
/**
 * @brief Initialize Control array with default AMD parameters
 *
 * Sets Control[AMD_DENSE] = 10.0 (dense threshold) and
 * Control[AMD_AGGRESSIVE] = 1 (enable aggressive absorption).
 *
 * @param Control Output array of size AMD_CONTROL to initialize
 */
void amd_defaults   (double Control [ ]) ;

/** @brief Initialize Control array (64-bit version) @see amd_defaults() */
void amd_l_defaults (double Control [ ]) ;

/**
 * @brief Print the current AMD control settings
 * @param Control Control array of size AMD_CONTROL (NULL prints defaults)
 */
void amd_control    (double Control [ ]) ;

/** @brief Print control settings (64-bit version) @see amd_control() */
void amd_l_control  (double Control [ ]) ;

/**
 * @brief Print AMD ordering statistics from Info array
 *
 * Prints matrix statistics, ordering quality metrics, and estimated
 * factorization costs from a previous amd_order call.
 *
 * @param Info Statistics array of size AMD_INFO from amd_order
 */
void amd_info       (double Info [ ]) ;

/** @brief Print statistics (64-bit version) @see amd_info() */
void amd_l_info     (double Info [ ]) ;

/**
 * @brief Get AMD library version numbers
 * @param version Output array filled with [major, minor, patch] version
 */
