...
/**
 * @brief Symbolic analysis: compute fill-reducing ordering and estimates
 *
 * Performs symbolic analysis of matrix A to compute a fill-reducing
 * column ordering and estimate memory requirements. The Symbolic object
 * can be reused for matrices with the same sparsity pattern.
 *
 * @param n_row Number of rows in A
 * @param n_col Number of columns in A
 * @param Ap Column pointers (size n_col+1)
 * @param Ai Row indices (size Ap[n_col])
 * @param Ax Numerical values (may be NULL for pattern-only analysis)
 * @param Symbolic Output: opaque symbolic object
 * @param Control Control parameters (NULL for defaults)
 * @param Info Output: statistics (NULL to ignore)
 * @return UMFPACK_OK on success, error code otherwise
 *
 * @see umfpack_di_numeric() for numerical factorization
 * @see umfpack_di_free_symbolic() to free returned object
 */
...
/**
 * @brief Numerical LU factorization using symbolic analysis
 *
 * Computes the numerical LU factorization P*R*A*Q = L*U using the
 * ordering from umfpack_di_symbolic. Uses multifrontal algorithm with
 * threshold partial pivoting.
 *
 * @algorithm Multifrontal LU with threshold pivoting and BLAS-3 kernels
 *
 * @param Ap Column pointers (size n_col+1)
 * @param Ai Row indices (size Ap[n_col])
 * @param Ax Numerical values (size Ap[n_col])
 * @param Symbolic Symbolic analysis from umfpack_di_symbolic
 * @param Numeric Output: opaque numeric factorization object
 * @param Control Control parameters (NULL for defaults)
 * @param Info Output: statistics (NULL to ignore)
 * @return UMFPACK_OK on success, error code otherwise
 *
 * @see umfpack_di_symbolic() for symbolic analysis
 * @see umfpack_di_solve() to solve using the factorization
 */
...
/**
 * @brief Solve linear system using LU factorization
 *
 * Solves Ax = b (or other systems) using the LU factorization from
 * umfpack_di_numeric. Can solve multiple system types: Ax=b, A'x=b,
 * A.'x=b, and partial solves with L, U, or permutations.
 *
 * @param sys System to solve: UMFPACK_A, UMFPACK_At, UMFPACK_Aat, etc.
 * @param Ap Column pointers (used for iterative refinement, or NULL)
 * @param Ai Row indices (used for iterative refinement, or NULL)
 * @param Ax Numerical values (used for iterative refinement, or NULL)
 * @param X Output: solution vector (size n)
 * @param B Input: right-hand side vector (size n)
 * @param Numeric LU factorization from umfpack_di_numeric
 * @param Control Control parameters (NULL for defaults)
 * @param Info Output: statistics (NULL to ignore)
 * @return UMFPACK_OK on success, error code otherwise
 *
 * @see umfpack_di_numeric() for computing the factorization
 */
...
/**
 * @brief Free memory allocated by umfpack_*_symbolic
 *
 * Deallocates the Symbolic object and sets the handle to NULL.
 * This is the only valid way to destroy a Symbolic object.
 *
 * @param Symbolic Pointer to Symbolic handle (set to NULL on output)
 */
...
/**
 * @brief Free memory allocated by umfpack_*_numeric
 *
 * Deallocates the Numeric object (LU factors) and sets the handle to NULL.
 * This is the only valid way to destroy a Numeric object.
 *
 * @param Numeric Pointer to Numeric handle (set to NULL on output)
 */
...
/**
 * @brief Initialize Control array with default parameter values
 *
 * Sets the Control array to UMFPACK's default settings. Call this
 * before modifying specific control parameters.
 *
 * @param Control Output: array of size UMFPACK_CONTROL with defaults
 *
 * @see UMFPACK_CONTROL_* defines for parameter indices
 */
...
/**
 * @brief Convert triplet (COO) format to compressed-column (CSC) format
 *
 * Converts a sparse matrix from (row,col,value) triplets to CSC format.
 * Duplicate entries are summed. The Map output allows tracking which
 * triplets contributed to each CSC entry.
 *
 * @param n_row Number of rows
 * @param n_col Number of columns
 * @param nz Number of triplet entries
 * @param Ti,Tj,Tx Row indices, column indices, values (triplet input)
 * @param Ap,Ai,Ax Column pointers, row indices, values (CSC output)
 * @param Map Optional: Map[k] gives destination index for triplet k
 * @return UMFPACK_OK on success
 *
 * @see cs_compress in CSparse for similar functionality
 */
...
/**
 * @brief Query sizes of L and U factors before extraction
 *
 * Returns dimensions and nonzero counts of the LU factorization,
 * needed for allocating arrays for umfpack_*_get_numeric.
 *
 * @param lnz Output: # nonzeros in L
 * @param unz Output: # nonzeros in U
 * @param n_row,n_col Output: matrix dimensions
 * @param nz_udiag Output: # nonzeros on diagonal of U
 * @param Numeric Input: Numeric object from umfpack_*_numeric
 * @return UMFPACK_OK on success
 */
