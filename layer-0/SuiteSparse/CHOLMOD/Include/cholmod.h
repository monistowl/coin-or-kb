 ...
 *
 * @see ldl.h for simple LDL' factorization
...
/**
 * @brief Initialize CHOLMOD and the Common workspace
 *
 * Must be called before any other CHOLMOD routine. Initializes control
 * parameters to defaults, allocates workspace, and sets up statistics.
 *
 * @param Common Workspace object to initialize
 * @return TRUE on success, FALSE on failure
 *
 * @see cholmod_finish() to free all workspace at end
 */
int cholmod_start (cholmod_common *Common) ;
/** @brief Initialize CHOLMOD (64-bit version) @see cholmod_start() */
...
/**
 * @brief Free all CHOLMOD workspace and finish
 *
 * Frees all workspace in Common. Should be called when done with CHOLMOD.
 *
 * @param Common Workspace to free
 * @return TRUE on success, FALSE on failure
 */
int cholmod_finish (cholmod_common *Common) ;
/** @brief Finish CHOLMOD (64-bit version) @see cholmod_finish() */
 ...
 *
 * Primary sparse matrix type for CHOLMOD. Supports packed (standard CSC) and
 * unpacked (gaps between columns) formats. Can hold symmetric matrices with
 * only upper or lower triangular part stored.
 *
 * Column j contains entries from p[j] to p[j+1]-1 (packed) or
 * p[j] to p[j]+nz[j]-1 (unpacked).
 *
 * @invariant p[0] == 0 for packed matrices
 * @invariant p[j] <= p[j+1] for all j (packed)
 *
 * @see cholmod_allocate_sparse() to create
 * @see cholmod_triplet for triplet (COO) format
 */
typedef struct cholmod_sparse_struct
{
    size_t nrow ;   /**< @brief Number of rows */
    size_t ncol ;   /**< @brief Number of columns */
    size_t nzmax ;  /**< @brief Maximum entries the matrix can hold */

    // int32_t or int64_t arrays:
    void *p ;       /**< @brief Column pointers (size ncol+1) */
    void *i ;       /**< @brief Row indices (size nzmax) */

    // for unpacked matrices only:
    void *nz ;      /**< @brief Number of entries per column (unpacked only, else NULL) */

    // double or float arrays:
    void *x ;       /**< @brief Numerical values (size nzmax or 2*nzmax for complex) */
    void *z ;       /**< @brief Imaginary part for zomplex (size nzmax, or NULL) */

    int stype ;     /**< @brief Symmetry: 0=unsym, >0=upper, <0=lower */
    int itype ;     /**< @brief Integer type: CHOLMOD_INT or CHOLMOD_LONG */
    int xtype ;     /**< @brief Value type: PATTERN, REAL, COMPLEX, or ZOMPLEX */
    int dtype ;     /**< @brief Precision: CHOLMOD_DOUBLE or CHOLMOD_SINGLE */
    int sorted ;    /**< @brief TRUE if row indices sorted within columns */
    int packed ;    /**< @brief TRUE if packed (nz ignored), FALSE if unpacked */
...
/**
 * @brief Sparse Cholesky factor (LL' or LDL')
 *
 * Holds symbolic or numeric factorization in either simplicial or supernodal
 * form. Simplicial factors use column-oriented storage like cholmod_sparse.
 * Supernodal factors group columns into dense submatrices (supernodes) for
 * better cache utilization.
 *
 * Factor types (determined by is_ll, is_super, xtype):
 * - Symbolic simplicial LDL' or LL': Only Perm, ColCount stored
 * - Symbolic supernodal LL': Perm, ColCount, plus supernode structure
 * - Numeric simplicial LDL': L with D on diagonal (unit L)
 * - Numeric simplicial LL': L with true diagonal
 * - Numeric supernodal LL': Dense supernodes in L
 *
 * @see cholmod_analyze() to create symbolic factor
 * @see cholmod_factorize() to compute numeric factor
 * @see cholmod_solve() to solve using the factor
 */
typedef struct cholmod_factor_struct
{
    size_t n ;      /**< @brief L is n-by-n */

    size_t minor ;  /**< @brief Column where failure occurred, or n if successful */
 ...
 *
 * Used for right-hand sides and solutions in cholmod_solve.
 * Stored column-major with optional leading dimension for submatrix views.
 *
 * @see cholmod_allocate_dense() to create
 * @see cholmod_solve() to solve with dense right-hand sides
 */
typedef struct cholmod_dense_struct
{
    size_t nrow ;       /**< @brief Number of rows */
    size_t ncol ;       /**< @brief Number of columns */
    size_t nzmax ;      /**< @brief Maximum entries (typically nrow*ncol) */
    size_t d ;          /**< @brief Leading dimension (d >= nrow) */
    void *x ;           /**< @brief Values (size d*ncol or 2*d*ncol for complex) */
    void *z ;           /**< @brief Imaginary for zomplex (size d*ncol, or NULL) */
    int xtype ;         /**< @brief CHOLMOD_REAL, COMPLEX, or ZOMPLEX */
    int dtype ;         /**< @brief CHOLMOD_DOUBLE or CHOLMOD_SINGLE */
...
/**
 * @brief Symbolic analysis: compute fill-reducing ordering and factor structure
 *
 * Orders the matrix A (or AA') using a fill-reducing ordering (AMD, COLAMD,
 * METIS, or nested dissection) and computes the symbolic structure of the
 * Cholesky factor L. The returned factor can be reused for multiple matrices
 * with the same sparsity pattern.
 *
 * @param A Sparse matrix to analyze (symmetric or rectangular)
 * @param Common Control parameters and workspace
 * @return Symbolic factor L, or NULL on error
 *
 * @see cholmod_factorize() to compute numerical factorization
 * @see cholmod_analyze_p() to provide a user permutation
 */
...
/** @brief Symbolic analysis (64-bit version) @see cholmod_analyze() */
...
/**
 * @brief Numerical Cholesky factorization using symbolic analysis
 *
 * Computes the numerical Cholesky factorization PAP' = LL' (or PAA'P' if
 * A->stype is 0) using a symbolic factor from cholmod_analyze. The analysis
 * can be reused for matrices with the same sparsity pattern.
 *
 * @algorithm Simplicial or supernodal Cholesky, depending on analysis
 *
 * @param A Sparse matrix to factorize (must match pattern from analyze)
 * @param L Factor object from cholmod_analyze (updated with numeric values)
 * @param Common Control parameters and workspace
 * @return TRUE on success, FALSE on failure (check Common->status)
 *
 * @note Failure indicates A is not positive definite; check L->minor
 * @see cholmod_analyze() for symbolic analysis
 * @see cholmod_solve() to solve using the factorization
 */
...
/** @brief Numerical factorization (64-bit version) @see cholmod_factorize() */
...
/**
 * @brief Solve linear systems using Cholesky factorization
 *
 * Solves various systems using the factorization L from cholmod_factorize.
 * The sys parameter selects which system to solve. D is identity for LL'.
 *
 * @param sys System to solve (see CHOLMOD_A, CHOLMOD_L, etc.)
 * @param L Cholesky factorization from cholmod_factorize
 * @param B Dense right-hand side matrix
 * @param Common Control parameters and workspace
 * @return Dense solution matrix X, or NULL on error
 *
 * @see cholmod_factorize() to compute the factorization
 * @see cholmod_solve2() for reusable workspace version
 */

#define CHOLMOD_A    0  /**< @brief Solve Ax=b (full system) */
#define CHOLMOD_LDLt 1  /**< @brief Solve LDL'x=b */
#define CHOLMOD_LD   2  /**< @brief Solve LDx=b */
#define CHOLMOD_DLt  3  /**< @brief Solve DL'x=b */
#define CHOLMOD_L    4  /**< @brief Solve Lx=b (forward substitution) */
#define CHOLMOD_Lt   5  /**< @brief Solve L'x=b (back substitution) */
#define CHOLMOD_D    6  /**< @brief Solve Dx=b (diagonal) */
#define CHOLMOD_P    7  /**< @brief Permute x=Px */
#define CHOLMOD_Pt   8  /**< @brief Permute x=P'x (inverse permutation) */
...
/** @brief Solve linear system (64-bit version) @see cholmod_solve() */
