 ...
 *
 * @see amd.h, colamd.h, cholmod.h, umfpack.h, klu.h for library-specific headers
 */
...
/**
 * @brief Initialize SuiteSparse global state
 *
 * Should be called once at application startup before using any SuiteSparse
 * library. Initializes internal state and default configuration. Thread-safe
 * initialization is handled internally.
 *
 * @see SuiteSparse_finish()
 */
void SuiteSparse_start ( void ) ;

/**
 * @brief Finalize SuiteSparse and release global resources
 *
 * Should be called once at application shutdown after all SuiteSparse
 * operations are complete. Releases any resources held by the configuration.
 *
 * @see SuiteSparse_start()
 */
void SuiteSparse_finish ( void ) ;

/**
 * @brief Allocate memory for an array of items using configured allocator
 *
 * Wrapper around the configured malloc function that enforces minimum
 * allocation of 1 item and handles size overflow checking.
 *
 * @param nitems Number of items to allocate (enforced >= 1)
 * @param size_of_item Size in bytes of each item (typically sizeof(type))
 * @return Pointer to allocated memory, or NULL on failure
 *
 * @see SuiteSparse_calloc(), SuiteSparse_free()
 */
...
/**
 * @brief Allocate and zero-initialize memory for an array of items
 *
 * Wrapper around the configured calloc function that enforces minimum
 * allocation of 1 item. All bytes are initialized to zero.
 *
 * @param nitems Number of items to allocate (enforced >= 1)
 * @param size_of_item Size in bytes of each item (typically sizeof(type))
 * @return Pointer to zero-initialized memory, or NULL on failure
 *
 * @see SuiteSparse_malloc(), SuiteSparse_free()
 */
...
/**
 * @brief Resize an existing memory allocation
 *
 * Wrapper around the configured realloc function with size tracking.
 * On failure, the original pointer remains valid and ok is set to 0.
 *
 * @param nitems_new New number of items desired
 * @param nitems_old Current number of items (for size calculation)
 * @param size_of_item Size in bytes of each item
 * @param p Pointer to existing allocation (may be NULL for initial alloc)
 * @param ok Output: set to 1 on success, 0 on failure
 * @return Pointer to reallocated memory, or original pointer on failure
 *
 * @see SuiteSparse_malloc(), SuiteSparse_free()
 */
...
/**
 * @brief Free memory allocated by SuiteSparse allocation functions
 *
 * Wrapper around the configured free function. Always returns NULL
 * to allow idiomatic "p = SuiteSparse_free(p)" usage pattern.
 *
 * @param p Pointer to memory block to free (NULL is safe)
 * @return Always returns NULL
 *
 * @see SuiteSparse_malloc(), SuiteSparse_calloc()
 */
 ...
 *
 * Records the current time for later elapsed time calculation via SuiteSparse_toc.
 * Uses the best available timer (OpenMP or clock_gettime).
 *
 * @param tic Output array of 2 doubles to store timer state
 *
 * @see SuiteSparse_toc(), SuiteSparse_time()
 */
...
/**
 * @brief Return elapsed time since the matching SuiteSparse_tic call
 *
 * @param tic Timer state from previous SuiteSparse_tic call
 ...
 *
 * Returns the current time from the system's high-resolution timer.
 * Uses OpenMP omp_get_wtime() if available, otherwise clock_gettime().
 *
 * @return Current wall-clock time in seconds (epoch-relative)
 */
...
/**
 * @brief Compute sqrt(x^2 + y^2) avoiding overflow and underflow
 *
 * @math Implements the hypot function: sqrt(x^2 + y^2) using a numerically
 *       stable algorithm that avoids intermediate overflow/underflow.
 *
 * @param x First value
 * @param y Second value
 * @return sqrt(x^2 + y^2) computed reliably
 */
double SuiteSparse_hypot (double x, double y) ;

/**
 * @brief Compute complex division c = a/b with numerical stability
 *
 * @math Computes (ar + ai*i) / (br + bi*i) = (cr + ci*i) using
 *       Smith's algorithm to avoid overflow in intermediate results.
 *
 * @param ar Real part of numerator a
 * @param ai Imaginary part of numerator a
 * @param br Real part of denominator b
 * @param bi Imaginary part of denominator b
 * @param cr Output: real part of result c
 * @param ci Output: imaginary part of result c
 * @return 1 on success, 0 if b is zero (division by zero)
 */
...
/**
 * @brief Get the SuiteSparse library version information
 *
 * Returns the version number as a single integer and optionally fills
 * an array with the three version components.
 *
 * @param version Optional output array for version components:
 *                version[0] = major, version[1] = minor, version[2] = patch.
 *                May be NULL if only the combined version is needed.
 * @return Combined version as (major * 1000 + minor)
 */
